...
const ProductCard = ({ data }) => {
  return (
    <Link to={`/product/${data.name_id}`} key={data.id} className="w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/5 p-2">
      <div className="bg-white rounded-lg hover:shadow-lg transition-transform hover:-translate-y-1 hover:scale-105 duration-300">
        ...
        <div className="p-2">
          <h6 className="text-sm font-medium text-center text-gray-800 truncate">{data.name}</h6>
        </div>
      </div>
    </Link>
  );
